...
export function AdminControls({ onVideoChange }: AdminControlsProps) {
  const [url, setUrl] = useState('')
  const [error, setError] = useState('')
  const [success, setSuccess] = useState(false)
  const [detectedType, setDetectedType] = useState<'youtube' | 'direct' | null>(
    null,
  )
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (isYouTubeUrl(url)) {
      ...
      setError(
        'Please use a YouTube link or direct video URL (.mp4, .webm, .ogg, .mov, .m3u8)',
      )
      return
    }
  ...
  return (
    <div className="bg-zinc-900 border-b border-zinc-800 p-4">
      <form
        ...
        className="flex gap-4 items-start max-w-4xl mx-auto"
      >
        <div className="flex-1">
          <div className="relative">
            <Input
              placeholder="YouTube link or Google Cloud Storage video URL"
              value={url}
              onChange={(e) => {
