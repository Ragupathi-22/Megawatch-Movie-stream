...
interface VideoPlayerProps {
  ...
  onUpdateState: (type: 'PLAY' | 'PAUSE' | 'SEEK', state: Partial<VideoState>) => void;
}
export function VideoPlayer({
  ...
  }, [videoState.playing, videoState.time, videoState.isYouTube, videoState.src]);
  ...
  if (!videoState.src) {
    return <div className="relative group bg-black aspect-video w-full rounded-xl overflow-hidden shadow-2xl ring-1 ring-zinc-800">
  ...
  return <div ref={containerRef} className="relative group bg-black aspect-video w-full rounded-xl overflow-hidden shadow-2xl ring-1 ring-zinc-800" onMouseMove={handleMouseMove} onMouseLeave={() => videoState.playing && setShowControls(false)}>
      {videoState.isYouTube ? <div id="youtube-player" className="w-full h-full" /> : <video ref={videoRef} src={videoState.src} className="w-full h-full object-contain" onTimeUpdate={handleTimeUpdate} onLoadedMetadata={handleLoadedMetadata} onWaiting={() => setIsLoading(true)} onPlaying={() => setIsLoading(false)} onCanPlay={() => setIsLoading(false)} onClick={handlePlayPause} crossOrigin="anonymous" />}

      {isLoading && <div className="absolute inset-0 flex items-center justify-center bg-black/20 backdrop-blur-sm z-10">
      ...
      <div className={`
        ...
        <div className="w-full mb-4 flex items-center gap-2">
          ...
          <input type="range" min={0} max={duration || 100} value={progress} onChange={handleSeek} className="flex-1 h-1 bg-white/20 rounded-full appearance-none cursor-pointer [&::-webkit-slider-thumb]:appearance-none [&::-webkit-slider-thumb]:w-3 [&::-webkit-slider-thumb]:h-3 [&::-webkit-slider-thumb]:rounded-full [&::-webkit-slider-thumb]:bg-violet-500 [&::-webkit-slider-thumb]:transition-transform hover:[&::-webkit-slider-thumb]:scale-125" />
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            <button onClick={handlePlayPause} className="p-2 rounded-full bg-white/10 hover:bg-white/20 text-white transition-colors">
              {videoState.playing ? <Pause size={24} fill="currentColor" /> : <Play size={24} fill="currentColor" />}
            </button>
            ...
            <div className="flex items-center gap-2 group/vol">
              ...
              {!videoState.isYouTube && <input type="range" min={0} max={1} step={0.1} value={isMuted ? 0 : volume} onChange={e => {
